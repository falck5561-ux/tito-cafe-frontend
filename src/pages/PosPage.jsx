...
function PosPage() {
  ...
  const handleAplicarRecompensa = (recompensa) => {
    ...
    if (nombreRecompensaLower.includes('gomita')) {
      const categoriaElegible = 'Gomita'; // Asegúrate que tus productos "Tito Pikulito" y "Tito Mojadito" tengan esta categoría
      
      ventaActual.forEach(item => {
        // Busca un item NO recompensa con la categoría correcta y el precio más alto
        if (item.categoria === categoriaElegible && !item.esRecompensa && Number(item.precioFinal) > precioMaximo) {
              ...
              <div className="card-body">
                ...
                <ul className="list-group list-group-flush" style={{ maxHeight: '300px', overflowY: 'auto' }}> {/* Scroll para tickets largos */}
                  ...
                  {ventaActual.map((item, index) => ( // Usar index para key si hay items repetidos (recompensa + normal)
                    <li key={`${item.id}-${index}`} className="list-group-item d-flex align-items-center justify-content-between p-1">
                      ...
                      {/* Botón Eliminar (deshabilitado para recompensa para evitar errores) */}
